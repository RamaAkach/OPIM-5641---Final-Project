# 🎶 Coachella & Glamour Portfolio — 2026-07-03

> **Strategy:** MPT with binary linking constraints, 60-day sliding window
> **Mode:** 🚀 MAX SHARPE
> **Stocks selected:** 10 / 25 &nbsp;|&nbsp; **Sectors:** 5

---

## 📊 Optimal Allocation (Linking Constraints: 5% ≤ X ≤ 40%)

| Stock | Ticker | Sector | Weight | Binary Y |
|-------|--------|--------|--------|----------|
| UnitedHealth | `UNH` | Healthcare | **33.15%** | ✅ 1 |
| SiriusXM | `SIRI` | Music | **26.85%** | ✅ 1 |
| Ralph Lauren | `RL` | Fashion | **5.00%** | ✅ 1 |
| Tapestry | `TPR` | Fashion | **5.00%** | ✅ 1 |
| Coty | `COTY` | Beauty | **5.00%** | ✅ 1 |
| Inter Parfums | `IPAR` | Beauty | **5.00%** | ✅ 1 |
| Live Nation | `LYV` | Music | **5.00%** | ✅ 1 |
| Johnson & J | `JNJ` | Healthcare | **5.00%** | ✅ 1 |
| Apple | `AAPL` | Tech | **5.00%** | ✅ 1 |
| Alphabet | `GOOGL` | Tech | **5.00%** | ✅ 1 |

**Stocks excluded (Y=0, X=0):**
`LVMUY`, `NKE`, `LULU`, `EL`, `ULTA`, `ELF`, `WMG`, `SPOT`, `IHRT`, `LLY`, `PFE`, `ABBV`, `MSFT`, `NVDA`, `META`

---

## 📈 Portfolio Performance

| Metric | Our Portfolio | S&P 500 (SPY) |
|--------|--------------|---------------|
| Annualised Return | **78.07%** | 26.30% |
| Annualised Risk   | 17.23% | 14.83% |
| Sharpe Ratio      | **4.240** | 1.436 |
| Max Drawdown      | — | -8.58% |

---

## 💰 Paper Trading (Simulated $10,000)

| Portfolio Value | SPY Value |
|----------------|-----------|
| **$10,425.32** | $10,059.12 |

Return vs Buy-and-Hold SPY: **4.25%** vs 0.59%

---

## 🌡️ Fear Index & Market Signals

**VIX Level:** 16.1 ✅ Normal — max-Sharpe mode active

**Golden Cross Signals (10d MA vs 30d MA):**

| Ticker | Signal |
|--------|--------|
| `LVMUY` | ⚪ Neutral |
| `RL` | 🟢 Bullish |
| `TPR` | 🟢 Bullish |
| `NKE` | 🔴 Bearish |
| `LULU` | 🔴 Bearish |
| `EL` | ⚪ Neutral |
| `ULTA` | ⚪ Neutral |
| `ELF` | 🟢 Bullish |
| `COTY` | 🟢 Bullish |
| `IPAR` | 🟢 Bullish |
| `LYV` | 🟢 Bullish |
| `WMG` | 🔴 Bearish |
| `SPOT` | 🔴 Bearish |
| `IHRT` | ⚪ Neutral |
| `SIRI` | 🟢 Bullish |
| `UNH` | 🟢 Bullish |
| `JNJ` | 🟢 Bullish |
| `LLY` | 🟢 Bullish |
| `PFE` | 🔴 Bearish |
| `ABBV` | 🟢 Bullish |
| `AAPL` | ⚪ Neutral |
| `MSFT` | 🔴 Bearish |
| `NVDA` | 🔴 Bearish |
| `META` | 🔴 Bearish |
| `GOOGL` | 🔴 Bearish |

---

## 📉 Charts

![Efficient Frontier](efficient_frontier.png)

![Optimal Allocation](allocation_pie.png)

![Paper Trading vs SPY](paper_trading.png)

![Weight History](weight_history.png)

---
*Auto-generated by GitHub Actions · 2026-07-03 · OPIM 5641*