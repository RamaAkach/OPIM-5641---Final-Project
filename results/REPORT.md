# 🎶 Coachella & Glamour Portfolio — 2026-08-10

> **Strategy:** MPT with binary linking constraints, 60-day sliding window
> **Mode:** 🚀 MAX SHARPE
> **Stocks selected:** 10 / 25 &nbsp;|&nbsp; **Sectors:** 5

---

## 📊 Optimal Allocation (Linking Constraints: 5% ≤ X ≤ 40%)

| Stock | Ticker | Sector | Weight | Binary Y |
|-------|--------|--------|--------|----------|
| UnitedHealth | `UNH` | Healthcare | **29.99%** | ✅ 1 |
| SiriusXM | `SIRI` | Music | **12.71%** | ✅ 1 |
| Nvidia | `NVDA` | Tech | **12.05%** | ✅ 1 |
| Microsoft | `MSFT` | Tech | **9.02%** | ✅ 1 |
| Eli Lilly | `LLY` | Healthcare | **8.83%** | ✅ 1 |
| e.l.f. Beauty | `ELF` | Beauty | **7.40%** | ✅ 1 |
| Ralph Lauren | `RL` | Fashion | **5.00%** | ✅ 1 |
| Tapestry | `TPR` | Fashion | **5.00%** | ✅ 1 |
| Inter Parfums | `IPAR` | Beauty | **5.00%** | ✅ 1 |
| Live Nation | `LYV` | Music | **5.00%** | ✅ 1 |

**Stocks excluded (Y=0, X=0):**
`LVMUY`, `NKE`, `LULU`, `EL`, `ULTA`, `COTY`, `WMG`, `SPOT`, `IHRT`, `JNJ`, `PFE`, `ABBV`, `AAPL`, `META`, `GOOGL`

---

## 📈 Portfolio Performance

| Metric | Our Portfolio | S&P 500 (SPY) |
|--------|--------------|---------------|
| Annualised Return | **103.57%** | 76.10% |
| Annualised Risk   | 16.04% | 14.19% |
| Sharpe Ratio      | **6.145** | 5.009 |
| Max Drawdown      | — | -4.49% |

---

## 💰 Paper Trading (Simulated $10,000)

| Portfolio Value | SPY Value |
|----------------|-----------|
| **$10,706.07** | $10,545.80 |

Return vs Buy-and-Hold SPY: **7.06%** vs 5.46%

---

## 🌡️ Fear Index & Market Signals

**VIX Level:** 14.9 ✅ Normal — max-Sharpe mode active

**Golden Cross Signals (10d MA vs 30d MA):**

| Ticker | Signal |
|--------|--------|
| `LVMUY` | ⚪ Neutral |
| `RL` | ⚪ Neutral |
| `TPR` | 🟢 Bullish |
| `NKE` | ⚪ Neutral |
| `LULU` | 🟢 Bullish |
| `EL` | 🟢 Bullish |
| `ULTA` | 🟢 Bullish |
| `ELF` | 🟢 Bullish |
| `COTY` | 🟢 Bullish |
| `IPAR` | 🟢 Bullish |
| `LYV` | 🟢 Bullish |
| `WMG` | ⚪ Neutral |
| `SPOT` | 🟢 Bullish |
| `IHRT` | 🔴 Bearish |
| `SIRI` | 🟢 Bullish |
| `UNH` | ⚪ Neutral |
| `JNJ` | 🟢 Bullish |
| `LLY` | ⚪ Neutral |
| `PFE` | 🟢 Bullish |
| `ABBV` | ⚪ Neutral |
| `AAPL` | 🟢 Bullish |
| `MSFT` | 🟢 Bullish |
| `NVDA` | 🟢 Bullish |
| `META` | 🔴 Bearish |
| `GOOGL` | ⚪ Neutral |

---

## 📉 Charts

![Efficient Frontier](efficient_frontier.png)

![Optimal Allocation](allocation_pie.png)

![Paper Trading vs SPY](paper_trading.png)

![Weight History](weight_history.png)

---
*Auto-generated by GitHub Actions · 2026-08-10 · OPIM 5641*